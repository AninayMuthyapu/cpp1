...
int main() {
    ...
    for(int r = 0; r < N; r++) { 
        for(int c = 0; c < N; c++) {
            if (c >= r) {
                ...
                h_B_s[r * N + c] = val; 
                h_B_s[c * N + r] = val; // Mirror for general validation
                h_B_s[c * N + r] = 0;   // Clear lower for strict UPLO check in cuBLAS
                h_B_s[r * N + c] = val; // Set Upper
                
                h_B_p[p_idx_b++] = val; // Packed
            } 
        }
    }
    ...
    for(int i=0; i<5; i++) {
        cublasSsymm(handle, CUBLAS_SIDE_LEFT, CUBLAS_FILL_MODE_LOWER, N, M, &alpha, d_B_s, N, d_A_s, K, &beta, d_C_cublas, N);
    }
