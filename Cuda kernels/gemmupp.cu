...
// // --- PACKED A LOADER (Updated) ---
// // Just like B, we now decompress A on the fly
...
//       // Scalar loads for Packed Layout
//       #pragma unroll
...
//               // UPPER TRIANGULAR CHECK FOR A (k >= m)
//               if (global_k >= global_m) {
//                   // Packed Offset Calculation for A
//                   size_t m_idx = (size_t)global_m;
//                   // Row Offset = sum(K-i for i in 0..m-1)
//                   size_t row_start = m_idx * K - (m_idx * (m_idx - 1)) / 2;
//                   size_t offset = row_start + (size_t)(global_k - global_m);
                  
//                   val = A[offset];
//               }
//           }
//           RegsA_Prefetch[mEl*VectorElems + v] = val;
//       }
//     }
//   }
// }
...
//             // UPPER TRIANGULAR CHECK FOR B (n >= k)
//             if (global_n >= global_k) {
//                 size_t k_idx = (size_t)global_k;
//                 size_t row_start = k_idx * N - (k_idx * (k_idx - 1)) / 2;
//                 size_t offset = row_start + (size_t)(global_n - global_k);
//                 val = B[offset];
//             }
//         }
//         RegsB_Prefetch[kEl*VectorElems + v] = val;
//     }
//   }
// }
...
// // ==============================================================================================
// // HOST CODE: Runner, Naive Kernel, and Main
// // ==============================================================================================

// // 1. Naive GEMM for Verification (Standard Dense Logic)
...
//         if (diff > 1.0) { 
//             printf(" Failed at index %d Ref: %f, Opt: %f (Diff: %f)\n", i, h_C_ref[i], h_C_opt[i], diff);
//             return;
//         }
//     }
...
//     // Calc Packed Sizes
//     size_t size_A_packed = ((size_t)M * (M + 1) / 2) * sizeof(float); 
//     size_t size_B_packed = ((size_t)N * (N + 1) / 2) * sizeof(float); 
    
//     // Dense Sizes for Verification
//     size_t size_A_dense = (size_t)M * K * sizeof(float);
//     size_t size_B_dense = (size_t)K * N * sizeof(float);
//     size_t size_C       = (size_t)M * N * sizeof(float);

//     std::vector<float> h_A_dense(M * K);
//     std::vector<float> h_A_packed((size_t)M * (M + 1) / 2);
    
//     std::vector<float> h_B_dense(K * N);
//     std::vector<float> h_B_packed((size_t)N * (N + 1) / 2);
...
//     // Initialize A (Upper Triangular)
//     size_t packed_idx_a = 0;
//     for(int r = 0; r < M; r++) {
//         for(int c = 0; c < K; c++) {
//             if (c >= r) {
//                 float val = (rand()%10)/10.0f;
//                 h_A_dense[r * K + c] = val;
//                 h_A_packed[packed_idx_a++] = val;
//             } else {
//                 h_A_dense[r * K + c] = 0.0f; 
//             }
//         }
//     }
//     printf("A Matrix Generated. Packed Elements: %lu\n", packed_idx_a);

//     // Initialize B (Upper Triangular)
//     size_t packed_idx_b = 0;
//     for(int r = 0; r < K; r++) {
//         for(int c = 0; c < N; c++) {
//             if (c >= r) {
//                 float val = (rand()%10)/10.0f;
//                 h_B_dense[r * N + c] = val;
//                 h_B_packed[packed_idx_b++] = val;
//             } else {
//                 h_B_dense[r * N + c] = 0.0f; 
//             }
//         }
//     }
//     printf("B Matrix Generated. Packed Elements: %lu\n", packed_idx_b);

//     float *d_A_dense, *d_A_packed, *d_B_dense, *d_B_packed, *d_C, *d_C_ref;
//     CUDA_ERROR_CHECK(cudaMalloc(&d_A_dense, size_A_dense));
//     CUDA_ERROR_CHECK(cudaMalloc(&d_A_packed, size_A_packed));
//     CUDA_ERROR_CHECK(cudaMalloc(&d_B_dense, size_B_dense));
//     CUDA_ERROR_CHECK(cudaMalloc(&d_B_packed, size_B_packed));
//     CUDA_ERROR_CHECK(cudaMalloc(&d_C, size_C));
//     CUDA_ERROR_CHECK(cudaMalloc(&d_C_ref, size_C));

//     CUDA_ERROR_CHECK(cudaMemcpy(d_A_dense, h_A_dense.data(), size_A_dense, cudaMemcpyHostToDevice));
//     CUDA_ERROR_CHECK(cudaMemcpy(d_A_packed, h_A_packed.data(), size_A_packed, cudaMemcpyHostToDevice));
//     CUDA_ERROR_CHECK(cudaMemcpy(d_B_dense, h_B_dense.data(), size_B_dense, cudaMemcpyHostToDevice));
//     CUDA_ERROR_CHECK(cudaMemcpy(d_B_packed, h_B_packed.data(), size_B_packed, cudaMemcpyHostToDevice));
//     CUDA_ERROR_CHECK(cudaMemset(d_C, 0, size_C));
//     CUDA_ERROR_CHECK(cudaMemset(d_C_ref, 0, size_C));

//     printf("Running Naive (Dense x Dense)...\n");
//     dim3 block(16, 16);
//     dim3 grid((N+15)/16, (M+15)/16);
//     naive_gemm<<<grid, block>>>(d_A_dense, d_B_dense, d_C_ref, M, N, K);
//     CUDA_ERROR_CHECK(cudaDeviceSynchronize());

//     printf("Running Optimized (Packed A x Packed B)...\n");
//     KernelOptimized<256, 128, 128, 16, 2> runner;
//     // Pass PACKED A and PACKED B
//     runner.run(0, d_A_packed, d_B_packed, d_C, M, N, K); 
//     CUDA_ERROR_CHECK(cudaDeviceSynchronize());
...
//     verify_result(h_C_ref.data(), h_C_opt.data(), M*N);

//     printf("Benchmarking Optimized Kernel (No Warmup)...\n");
//     cudaEvent_t start, stop;
//     cudaEventCreate(&start); cudaEventCreate(&stop);
    
//     cudaEventRecord(start);
//     int iters = 20;
//     for(int i=0; i<iters; i++) runner.run(0, d_A_packed, d_B_packed, d_C, M, N, K);
//     cudaEventRecord(stop);
//     cudaEventSynchronize(stop);
    
//     float ms; cudaEventElapsedTime(&ms, start, stop);
//     double avg_ms = ms / iters;
//     double gflops = (2.0 * M * N * K * 1e-9) / (avg_ms / 1000.0);
    
//     printf("Avg Time: %.3f ms | Performance: %.2f GFLOPS\n", avg_ms, gflops);

//     cudaFree(d_A_dense); cudaFree(d_A_packed); 
//     cudaFree(d_B_dense); cudaFree(d_B_packed); 
//     cudaFree(d_C); cudaFree(d_C_ref);
...
int main() {
    ...
    dim3 block(16, 16);
    dim3 grid((N+15)/16, (M+15)/16);
    naive_gemm<<<grid, block>>>(d_A_s, d_B_s, d_C_ref, M, N, K);
    CUDA_ERROR_CHECK(cudaDeviceSynchronize());
