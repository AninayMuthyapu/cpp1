...
void matmul_op(float* out, const float* a, const float* b, int m, int n, int k) {{
    for (int i = 0; i < m; ++i) {{
        for (int j = 0; j < n; ++j) {{
            out[i*n + j] = 0;
            for (int l = 0; l < k; ++l) {{
...
""")
    ...
    for op in ops_sorted:
        ...
        if op_type_lower == "add":
            ...
            code_parts.append(f"    matmul_op({out_cpp_name}, {lhs_name}, {rhs_name}, {m_dim_cpp}, {n_dim_cpp}, {k_dim_cpp});\n")
        else:
